...
def apply_wavelet_equalization(signal, sr, slider_settings, gains, wavelet='db4', level=None):
    ...
    if not HAS_PYWT:
        return signal, None, None, None, None
    ...
    # For display, compute FFT of both
    n = len(signal)
    freqs = np.fft.rfftfreq(n, d=1.0 / sr)
    input_fft = np.abs(np.fft.rfft(signal))
    output_fft = np.abs(np.fft.rfft(output))

    return output, freqs, input_fft, output_fft, None
...
def equalize():
    ...
    try:
        ...
        if transform == 'fourier':
            ...
            output, freqs, input_mag, output_mag, _ = \
                apply_wavelet_equalization(sig, sr, settings_sliders, gains, wavelet=transform)
        print(f"[EQ] success, output len={len(output)}, freqs={'None' if freqs is None else len(freqs)}")

        store['output'] = output

        # Downsample frequency data for transfer
